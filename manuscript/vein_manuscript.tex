...
real on-chain data around the October~2025 cascading liquidation event, one of
the largest documented deleveraging episodes in crypto markets, with generality checks on the
FTX (2022) and USDC/SVB (2023) stress episodes. The primary contribution is the
counterfactual attribution measure, which yields an exact, order-robust (Shapley)
decomposition of realised losses into upstream entities and produces
systemic-importance rankings that diverge from $\Delta$CoVaR and MES (Spearman
$\rho=0.52$), survive removing the retail hub, and are stable under coefficient
perturbation (Spearman $0.88$). Prediction is a secondary, graph-source
ablation: at the cascade's hourly timescale the observed graph is competitive
with a price-estimated Granger graph (better in $71\%$ of paired bootstrap
resamples) but the edge is not decisive---it does not survive a strict
lagged-flow forecast or a price-based stress target---so we report H1 as
suggestive rather than established. Flow-direction's causal content is event- and
target-conditional but real: edge reversal degrades one-step prediction in the
FTX and USDC/SVB episodes and, crucially, by $\approx9\%$ on a leakage-free
non-flow stress target, confirming the directional signal is not an artefact of
flow-defined stress; it washes out only in the custodial-flow-dominated
October-2025 cascade. We therefore qualify the directional reading to the edge
types and events where it is identified. A causal-validity battery
(placebo/negative controls, unobserved-confounding robustness values, resolution
robustness, and external validity against the documented narrative) supports
these components while delimiting the off-chain-CEX and entity-resolution limits. We discuss the implications for regulators and the limits imposed
